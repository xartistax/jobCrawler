...
function SavedJobsDashboard() {
  ...
  if (loading)
    return (
      <p className="p-6 text-gray-500 text-xs">
        Gespeicherte Jobs werden geladen…
      </p>
    );

  if (savedJobs.length === 0)
    return (
      <p className="p-6 text-gray-500 text-xs">Keine gespeicherten Jobs.</p>
    );
      ...
      <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-3">
        {savedJobs.map((savedJob) => (
          <div key={savedJob.id} className="flex">
            <JobCard allowUnsave job={savedJob.job} />
          </div>
        ))}
      </div>
    </section>
