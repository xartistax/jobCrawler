        ...
        <div className="mb-12 flex items-center justify-center gap-3 text-xs font-extralight text-default-500">
          ...
          <div className="flex gap-2">
            {siteConfig.fullNavItems.map((item) => (
              <Link key={item.href} href={item.href} className="text-primary after:content-['|'] after:ml-2 last:after:content-none text-xs">
                {item.label}
              </Link>
            ))}
          </div>
          ...
          <div className="w-full grid grid-cols-4 place-items-center gap-5 text-black dark:text-default-200 sm:grid-cols-5 lg:grid-cols-4 lg:w-6/12">
            {siteConfig.frontIconNav.map((item) => (
              <Tooltip content={item.label} key={item.label}>
                <Link href={item.href} aria-label={item.label} isExternal className="text-inherit">
                  <div title={item.label} className="scale-80 sm:scale-100">
                    <item.icon size={36} />
                  </div>
                </Link>
              </Tooltip>
